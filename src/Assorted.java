...
public class Assorted {
    ...
    public static int findSum(List<?> list) {
        ...
        for(int i=0; i<list.size();i++){
            ...
            else if(list.get(i).getClass().getSimpleName().equals("Integer")){
                sum+=(int)list.get(i);
            }
        }
        return sum;
    ...
    public static int busStop(List<Integer[]> list) {
        int count = 0;
        for (Integer[] i : list) {
            //aha this short version of for loops is so much less annoying to type out
            count+= i[0];
            count-= i[1];
        }
    ...
    public static List<Integer> subtractList(List<Integer> listA, List<Integer> listB) {
        ...
        } //i learned removeAll of alvin, credit to him
        return listA;
    }
    /**
     * Challenge 7
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        Map<Integer, Integer> countount = new HashMap<>();
        List<Integer> filteredList = new ArrayList<>();

        for (Integer motif : list) {
            int count = countount.getOrDefault(motif, 0);
            if (count < n) {
                filteredList.add(motif);
                countount.put(motif, count + 1);
            }
        }
